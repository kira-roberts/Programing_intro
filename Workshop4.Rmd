...
The following is a record of what was undertaken and the code necessary for this. Most code was run through the console with some exceptions in the terminal. This file should not be run as is.
...
#GEMINI_API_KEY="AIzaSyBtyTCeAiyBkH8EfpUkR3JwcKEvpfa6vXE"
...
iris %>% 
  ...
  map(~summary(.x))
```
## Task using chattr: (ChatGPT at the moment)

Use chattr to convert a complex operation. Prompt the engine: "I have a list of three vectors containing fish count data. Write a for-loop to calculate the mean of each, and then show me the exact parallel way to do it using purrr's map_dbl function." Compare the visual simplicity. The code given is below
```{r}
